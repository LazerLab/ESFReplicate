...
transformed parameters {
  ...
  a_vint <- a_s_vint * (a_e_vint - mean(a_e_vint));
  a_Interest_pol <- a_s_Interest_pol * (a_e_Interest_pol - mean(a_e_Interest_pol));
  a_Ideology <- a_s_Ideology * (a_e_Ideology - mean(a_e_Ideology));
  a_Party <- a_s_Party * (a_e_Party - mean(a_e_Party));
  a_Race <- a_s_Race * (a_e_Race - mean(a_e_Race));
  a_SchYear <- a_s_SchYear * (a_e_SchYear - mean(a_e_SchYear));
  a_ChapterN <- a_s_ChapterN * (a_e_ChapterN - mean(a_e_ChapterN));
  for (n in 1:N) {
  ...
  linear_predictor[n] <- a0 +
    ...
    g0 * Female[n] +
    b0[1] * mvint_pol_both[n] +
    b0[2] * ideomd_pol_both[n] +
    b0[3] * ideosd0_pol_both[n] +
    b0[4] * party_rhhi_pol_both[n];
  }
}
model {
  a0 ~ normal(0, 10);
